...
export function getLodgingSuggestions(placeName: string, category: 'Lodging' | 'Camping', lang: 'fr' | 'th'): LodgingProposal[] {
  ...
  if (category === 'Camping') {
    ...
    if (placeName.includes("Kanchanaburi") || placeName.includes("Erawan") || placeName.includes("Sai Yok") || placeName.includes("Srinakarin")) {
      lowName = lang === 'fr' ? "River Kwai Jungle Rafts Hostel" : "แพริเวอร์แคว จังเกิ้ลราฟท์";
      midName = lang === 'fr' ? "U Inchantree Kanchanaburi" : "ยู อินจันทรี กาญจนบุรี";
      highName = lang === 'fr' ? "The Float House River Kwai" : "เดอะ โฟลทเฮ้าส์ ริเวอร์แคว รีสอร์ท";
    } else if (placeName.includes("Ratchaburi") || placeName.includes("Suan Phueng") || placeName.includes("Khao Bin")) {
      lowName = lang === 'fr' ? "Suan Phueng Mountain Guesthouse" : "สวนผึ้ง เมาน์เทน เกสต์เฮาส์";
      midName = lang === 'fr' ? "Aristo Chic Resort & Farm Suan Phueng" : "อริสโต ชิค รีสอร์ท แอนด์ ฟาร์ม";
      highName = lang === 'fr' ? "La Toscana Resort Ratchaburi" : "ลา ทอสคาน่า รีสอร์ท ราชบุรี";
    } else if (placeName.includes("Saraburi") || placeName.includes("Chet Sao Noi") || placeName.includes("Sam Lan") || placeName.includes("Muak Lek")) {
      lowName = lang === 'fr' ? "Muaklek Forest Guesthouse" : "มวกเหล็ก ฟอเรสต์ เกสต์เฮาส์";
      midName = lang === 'fr' ? "Baan Dara Resort Saraburi" : "บ้านดารา รีสอร์ท สระบุรี";
      highName = lang === 'fr' ? "Ndol Streamside Thai Villas" : "ณดล สตรีมไซด์ ไทย วิลล่า";
    } else if (placeName.includes("Nakhon Nayok") || placeName.includes("Khun Dan") || placeName.includes("Sarika") || placeName.includes("Wang Takrai")) {
      lowName = lang === 'fr' ? "Wang Takrai Riverside Bungalow" : "วังตะไคร้ ริเวอร์ไซด์ บังกะโล";
      midName = lang === 'fr' ? "Sida Resort Nakhon Nayok" : "สีดา รีสอร์ท นครนายก";
      highName = lang === 'fr' ? "At Rice Resort Nakhon Nayok" : "แอท ไรซ์ รีสอร์ท นครนายก";
    } else if (placeName.includes("Bangkok") || placeName.includes("Samut Sakhon")) {
                      ...
                      <div className="flex items-stretch cursor-pointer">
                        ...
                        <div className="flex-1 p-3.5 space-y-1.5">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="flex items-center gap-1 opacity-60 group-hover/card:opacity-100 transition-opacity ml-2">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="p-2.5 lg:p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded disabled:opacity-30 cursor-pointer"
                              >
                                <ArrowUp className="w-4 h-4 lg:w-3.5 lg:h-3.5" />
                              </button>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="p-2.5 lg:p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded disabled:opacity-30 cursor-pointer"
                              >
                                <ArrowDown className="w-4 h-4 lg:w-3.5 lg:h-3.5" />
                              </button>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="p-2.5 lg:p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded cursor-pointer"
                              >
                              ...
                              <button
                                ...
                                className="p-2.5 lg:p-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded cursor-pointer"
                              >
